<template>
    <div>
        ...
        <a-modal :visible="dialogVisible" :title="'图片裁剪'" :width="'40%'" :custom-class="'upload_dialog'"
            @close="dialogVisible = false">

            <div class="cropper">
                <div class="cropper_left">
                    <vueCropper :tyle="{ width: '400px' }" ref="cropperRef" :img="options.img" :info="true"
            ...
            <template #footer>
                <span class="dialog-footer">
                    ...
                    <a-button type="primary" @click="refreshCrop">重置</a-button>
...
const onConfirm = () => {
    cropperRef.value.getCropData(async (data: string) => {
        ...
        const res = await cropperSuccess(dataFile).then(res => {
            console.log(res);
            return res
        })
        // 触发自定义事件
